...
const ModuleList = ({sequence_modules, executionPanelRef}) => {
    ...
    const listHeaders = Object.entries(sequences_modules_obj).flatMap(([moduleKey,sequences]) =>
        <SequenceModule key={moduleKey} header={moduleKey} sequences={JSON.stringify(sequences)} executionPanelRef={executionPanelRef}></SequenceModule>
    )
