...
and then unpackage that with ```tar -xvzf``` to use a small test application to see the components in-use. 

When doing this, ensure you edit the ``static_path`` in your ``.cfg`` file to reference the resulting ``dist/`` directory from the ``.tgz``.

Then you can access the UI at:
