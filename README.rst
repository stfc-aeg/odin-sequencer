...
Alternatively, run command `wget https://github.com/stfc-aeg/odin-sequencer-ui/releases/download/<version>/app_build.tgz` and then unpackage that with `tar -xvzf` to use a small test application to see the components in-use. When doing this, ensure you edit the `static_path` in your `.cfg` file to reference the resulting `dist/` directory from the `.tgz`.
